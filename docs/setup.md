...
#### Run the API
- Using helper script (auto‑loads `.env`, opens auth by default):
  ```bash
  ./start.sh               # first time: chmod +x start.sh
  # options:
  ./start.sh --open=swagger   # open Swagger UI instead
  ```
- Manual commands:
  ```bash
  dotnet restore
...
#### Troubleshooting
- Missing framework “Microsoft.NETCore.App 8.x”: ensure .NET 8 SDK/Runtime is installed (see above). A `global.json` pins SDK 8.x in this repo.
